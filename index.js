...
conmenu = fs.readFileSync('./X2/Ramdani.jpeg')
...
audio20 = fs.readFileSync('./X3/nayla20.m4a') 
            ...
            const nnnyz6 = {contentText: `[ *SELAMAT DATANG* ]\n${tz} *NAME* : @${memJid.split('@')[0]}\n${tz} *GROUP* : ${mdata.subject}\n${tz} *MEMBER* : ${jumlahMem.length}`, footerText: `© Creator by Ramdani Official || 2022`, buttons: nnnyz5, headerType: 6, locationMessage: nnnyz4.message.locationMessage}
		...
		//	if (nay.key.fromMe) return
		    ...
		    const A1 = [
            {buttonId: 'id1', buttonText: {displayText: `IYA`}, type: 1},            
			...
			if ((args[0]) === 'on') {
			if (isAntiLink) return reply(`*BERHASIL MENGAKTIFKAN FITUR ANTILINK DI GRUP INI✅*`)
			antilink.push(from)
			fs.writeFileSync('./X2/antilink.json', JSON.stringify(antilink))
			reply(`*BERHASIL MENGAKTIFKAN FITUR ANTILINK DI GRUP INI✅*`)
			} else if ((args[0]) === 'off') {
			if (!isAntiLink) return reply(`*BERHASIL MENONAKTIFKAN FITUR ANTILINK DI GRUP INI✅*`)
			antilink.splice(from, 1)
			fs.writeFileSync('./X2/antilink.json', JSON.stringify(antilink))
			reply(`*BERHASIL MENONAKTIFKAN FITUR ANTILINK DI GRUP INI✅*`)
			} else {
		    ...
		    case 'menu': case 'help':
		    if (isBanned) return reply("MAAF ANDA TELAH DI BANNED")
		  	Reply("MENAMPILKAN MENU ZUXYYI BOT")	
			menu3 = `⚯⚯[ *ZUXYY BOTZ* ]⚯⚯
HALOO!! ${codename}👋
...
⚯⚯[ *THX TO* ] <[ *5* ]>⚯⚯
[ *${tz}* ] ➙ *RAMDANI OFFICIAL [ AUTHOR ]*
[ *${tz}* ] ➙ *VANNESS TZY(RECODE)*
[ *${tz}* ] ➙ *NAYLA*
[ *${tz}* ] ➙ *LORD RIYANN* 
[ *${tz}* ] ➙ *LOLYKILLERS*
...
⚯⚯⚯⚯⚯⚯⚯⚯⚯⚯⚯`    
            ...
            } catch (e) { console.log(color("JIKA INGIN MENAMBAHKAN AUDIO/SOUND PADA MENU, SILAHKAN ADD/TEMPEL AUDIO KALIAN DI FOLDER X2, LALU UBAH NAMA SOUND MENJADI menu.mp3", 'green'))}            
            const menu1 = [
